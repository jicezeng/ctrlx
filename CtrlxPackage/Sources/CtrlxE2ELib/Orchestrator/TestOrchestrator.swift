...
public actor TestOrchestrator {
    ...
    /// Xcode workspace), so it lives under `CtrlxPackage/.build/{debug,release}/`.
    /// We assemble candidate `CtrlxPackage` roots from several sources and
    ...
    private func locateEchoSidecarBinary(sourceFile: String = #file) throws -> URL {
        ...
        /// Walk up from `start` until a directory containing `CtrlxPackage/Package.swift`
        /// (repo root) OR `Package.swift` directly (the package root itself) is found.
        /// Returns the `CtrlxPackage` directory in either case.
        func packageRoot(from start: URL) -> URL? {
            var dir = start
            for _ in 0..<20 {
                let nested = dir.appendingPathComponent("CtrlxPackage/Package.swift")
                if fm.fileExists(atPath: nested.path) {
                    return dir.appendingPathComponent("CtrlxPackage")
                }
                ...
                if
                    ...
                    dir.lastPathComponent == "CtrlxPackage" {
                    return dir
                }
        ...
        throw OrchestratorError.configurationError(
            "EchoPluginSidecar binary not found. Searched: \(searched). " +
                "Run `swift build` in CtrlxPackage first " +
                "(or set CTRLX_PACKAGE_ROOT to the CtrlxPackage directory)."
        )
    }
